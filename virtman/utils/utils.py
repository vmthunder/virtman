...
def reform_connections(connections):
    ...
    if not (isinstance(connections, list) or isinstance(connections, tuple)):
        raise Exception('Virtman: Unknown connections type: connection: {0:s}, '
                        'type: {1:s}'.format(connections, type(connections)))
    new_connections = []
...
def reform_connection(connection):
    """
    :return new_connection: a list includes 'target_portal', 'target_iqn' and
    'target_lun'.
    """
