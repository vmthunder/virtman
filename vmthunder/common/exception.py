...
"""vmthunder base exception handling.

Includes decorator for re-raising vmthunder-type exceptions.
...
class VMThunderException(Exception):
    """Base vmthunder Exception
    ...
    def __init__(self, message=None, **kwargs):
        ...
        self.msg = message
        super(VMThunderException, self).__init__(message)

    def __unicode__(self):
        return unicode(self.msg)


class VolumeBackendAPIException(VMThunderException):
    message = _("Bad or unexpected response from the storage volume "
                "backend API: %(data)s")


class VolumeDriverException(VMThunderException):
    message = _("Volume driver reported an error: %(message)s")


class BackupDriverException(VMThunderException):
    message = _("Backup driver reported an error: %(message)s")


class GlanceConnectionFailed(VMThunderException):
    message = _("Connection to glance failed: %(reason)s")


class NotAuthorized(VMThunderException):
...
class ImageNotAuthorized(VMThunderException):
    message = _("Not authorized for image %(image_id)s.")


class DriverNotInitialized(VMThunderException):
    message = _("Volume driver not ready.")


class Invalid(VMThunderException):
...
class SfJsonEncodeFailure(VMThunderException):
...
class NotFound(VMThunderException):
...
class VolumeTypeInUse(VMThunderException):
...
class VolumeIsBusy(VMThunderException):
    message = _("deleting volume %(volume_name)s that has snapshot")


class SnapshotIsBusy(VMThunderException):
...
class OverQuota(VMThunderException):
...
class Duplicate(VMThunderException):
    pass
...
class MalformedRequestBody(VMThunderException):
...
class NoValidHost(VMThunderException):
    message = _("No valid host was found. %(reason)s")


class NoMoreTargets(VMThunderException):
    """No more available targets."""
    pass


class QuotaError(VMThunderException):
...
class VolumeTypeCreateFailed(VMThunderException):
...
class VolumeMigrationFailed(VMThunderException):
    message = _("Volume migration failed: %(reason)s")


class SSHInjectionThreat(VMThunderException):
...
class QoSSpecsCreateFailed(VMThunderException):
    message = _("Failed to create qos_specs: "
                "%(name)s with specs %(qos_specs)s.")


class QoSSpecsUpdateFailed(VMThunderException):
...
class QoSSpecsAssociateFailed(VMThunderException):
    message = _("Failed to associate qos_specs: "
                "%(specs_id)s with type %(type_id)s.")


class QoSSpecsDisassociateFailed(VMThunderException):
...
class QoSSpecsInUse(VMThunderException):
    message = _("QoS Specs %(specs_id)s is still associated with entities.")


class KeyManagerError(VMThunderException):
...
class CoraidException(VolumeDriverException):
    message = _('Coraid vmthunder Driver exception.')
...
class ZadaraException(VolumeDriverException):
    message = _('Zadara vmthunder Driver exception.')
...
class SolidFireDriverException(VolumeDriverException):
    message = _("SolidFire vmthunder Driver exception")
